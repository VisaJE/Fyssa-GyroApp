...
CREATE_GROUP(FYSSA_PATH_POSARRAY_,
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_X),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_Y),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_Z));

CREATE_GROUP(FYSSA_PATH_1,
    static_cast<uint16_t>(ARRAY_BEGIN),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_),
    static_cast<uint16_t>(ARRAY_END));

CREATE_GROUP(FYSSA_PATH_2,
    static_cast<uint16_t>(ARRAY_BEGIN),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_),
    static_cast<uint16_t>(ARRAY_END));

CREATE_GROUP(FYSSA_PATH_3,
    static_cast<uint16_t>(ARRAY_BEGIN),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_),
    static_cast<uint16_t>(FYSSA_PATH_POSARRAY_),
...
const DescriptorItem_t s_possibleSbemItems[] = {
    ...
    {FYSSA_PATH_POSARRAY_X, "<PTH>Fyssa.Path.PosArray+X\n<FRM>float32"},
    {FYSSA_PATH_POSARRAY_Y, "<PTH>Fyssa.Path.PosArray.Y\n<FRM>float32"},
    {FYSSA_PATH_POSARRAY_Z, "<PTH>Fyssa.Path.PosArray.Z\n<FRM>float32"},
...
size_t writeToSbemBuffer_FYSSA_PATH(void *buffer, size_t bufferLen, size_t startOffset, const WB_RES::LOCAL::FYSSA_PATH::EVENT::NotificationType &data){
    size_t bytesRead = 0;
    size_t bytesWritten = 0;
    uint8_t *byteBuf = static_cast<uint8_t*>(buffer);
    for(size_t loopidx_1=0; loopidx_1<data.posArray.size(); loopidx_1++)
    {
        {
            auto value = data.posArray[loopidx_1].x;
            const uint8_t *src = reinterpret_cast<const uint8_t*>(&value);
            for(size_t i=0; i<sizeof(value); i++, src++, bytesRead++)
                if ((bytesRead >= startOffset) && (bytesWritten < bufferLen)) byteBuf[bytesWritten++] = *src;
        }
        {
            auto value = data.posArray[loopidx_1].y;
            const uint8_t *src = reinterpret_cast<const uint8_t*>(&value);
            for(size_t i=0; i<sizeof(value); i++, src++, bytesRead++)
                if ((bytesRead >= startOffset) && (bytesWritten < bufferLen)) byteBuf[bytesWritten++] = *src;
        }
        {
            auto value = data.posArray[loopidx_1].z;
            const uint8_t *src = reinterpret_cast<const uint8_t*>(&value);
            for(size_t i=0; i<sizeof(value); i++, src++, bytesRead++)
                if ((bytesRead >= startOffset) && (bytesWritten < bufferLen)) byteBuf[bytesWritten++] = *src;
        }
...
size_t getSbemLength_FYSSA_PATH(const WB_RES::LOCAL::FYSSA_PATH::EVENT::NotificationType &data)
{
    return (0 + data.posArray.size() * (0  + 4 + 4 + 4 ) );
}
...
size_t getSbemLength(whiteboard::LocalResourceId localResId, const whiteboard::Value &data)
{
    ...
    case WB_RES::LOCAL::FYSSA_PATH::LID:
        return getSbemLength_FYSSA_PATH(data.convertTo<WB_RES::LOCAL::FYSSA_PATH::EVENT::NotificationType>());
...
size_t writeToSbemBuffer(void *buffer, size_t bufferLen, size_t startOffset, whiteboard::LocalResourceId localResId, const whiteboard::Value &data)
{
    ...
    case WB_RES::LOCAL::FYSSA_PATH::LID:
        return writeToSbemBuffer_FYSSA_PATH(buffer, bufferLen, startOffset, data.convertTo<WB_RES::LOCAL::FYSSA_PATH::EVENT::NotificationType>());
    case WB_RES::LOCAL::MEAS_ACC_SAMPLERATE::LID:
...
const SbemResID2GrpIdMapping s_sbemResID2GrpIdMap[] = {
    ...
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (1)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_1_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (2)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_2_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (3)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_3_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (4)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_4_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (5)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_5_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (6)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_6_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (7)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_7_Group },
    {WB_RES::LOCAL::FYSSA_PATH::LID,  +  (8)  * (0  + 4 + 4 + 4 ) , FYSSA_PATH_8_Group },
